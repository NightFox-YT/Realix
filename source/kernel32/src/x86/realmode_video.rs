...
// ❗️ Смена видеорежима - синхронная запись в регистры VGA-контроллера
//    внутри самого BIOS, без DMA/аппаратных прерываний завершения - в
//    отличие от дисковых BIOS-вызовов, здесь не нужно ни включать
//    прерывания (sti), ни трогать PIC/PIT. Прерывания остаются выключены
//    (cli) на всё время перехода.
