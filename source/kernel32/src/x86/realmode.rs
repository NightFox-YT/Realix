...
fn bios_disk_call(operation: u8, lba: u32, count: u8, dest: u32) -> bool {
    unsafe {
        ...
        // маски раньше чинило зависание, но снятая маска IRQ1 (клавиатура)
        // отправляла нажатые во время вызова клавиши в собственный буфер
        // BIOS вместо очереди kernel32 - они терялись для shell. С IRQ6 как
        // единственным снятым - нажатия остаются отложенными (8259A
        // защёлкивает IRQ по фронту даже в маске, см. IRR), доходя до
        // kernel32 сразу после восстановления обычной маски по возврату
        //
        // ❗️ ОТДЕЛЬНО от маскирования: сам PIT переключаем на "родной"
        // делитель BIOS (~18.2 Гц вместо 100 Гц kernel32) на время вызова.
        // Это не про доставку прерываний - маскирование IRQ0 её и так не
        // пускает - а про то, что BIOS-код записи на дискету (раскрутка
        // мотора, таймауты) сам читает СЧЁТЧИК PIT напрямую или считает
        // ожидаемые тики, предполагая родную частоту; на 100 Гц (в 5.5 раза
        // чаще) этот отсчёт сбивается независимо от прерываний - вероятная
        // причина, почему запись зависала там, где чтение уже работало
        //
        ...
        crate::drivers::pit::use_bios_rate();

        rm_bios_disk_call();

        crate::drivers::pit::restore_rate();
