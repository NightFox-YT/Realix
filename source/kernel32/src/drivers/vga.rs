...
pub fn clear_screen() {
    ...
    unsafe { CURSOR_ROW = 0; CURSOR_COL = 0; }
}
...
pub fn put_char(c: u8, color: Color) {
    ...
    unsafe {
        ...
        if CURSOR_COL >= VGA_WIDTH { CURSOR_COL = 0; CURSOR_ROW += 1; }
        while CURSOR_ROW >= VGA_HEIGHT { scroll_up(); CURSOR_ROW = VGA_HEIGHT - 1; }
        update_cursor();
    }
}

pub fn print_str(s: &str, color: Color) {
    for byte in s.bytes() { put_char(byte, color); }
}

pub fn backspace() {
    unsafe {
        if CURSOR_COL > 0 { CURSOR_COL -= 1; }
        else if CURSOR_ROW > 0 { CURSOR_ROW -= 1; CURSOR_COL = VGA_WIDTH - 1; }
        let offset = (CURSOR_ROW * VGA_WIDTH + CURSOR_COL) * 2;
...
pub fn hide_cursor() {
    unsafe {
        core::arch::asm!("out dx, al", in("dx") 0x3D4u16, in("al") 0x0Au8);
        core::arch::asm!("out dx, al", in("dx") 0x3D5u16, in("al") 0x20u8);
    }
}
