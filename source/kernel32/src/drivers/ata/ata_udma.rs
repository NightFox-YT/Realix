/* The ATA driver on rust for Realix, built on UDMA
...
pub unsafe fn pci_find_ide() -> Option<PciIdeInfo> 
{
    for bus in 0u16..256 
    {
        let bus = bus as u8;
        for slot in 0u8..32 {
            for func in 0u8..8 {
                let vendor_dev = pci_read_config(bus, slot, func, PCI_VENDOR_DEVICE);
                ...
                let class_rev = pci_read_config(bus, slot, func, PCI_CLASS_REV);
                let class_code = ((class_rev >> 24) & 0xFF) as u8;
                let subclass = ((class_rev >> 16) & 0xFF) as u8;

                if class_code == PCI_MASS_STORAGE && subclass == PCI_SUBCLASS_IDE {
                    let cmd = pci_read_config(bus, slot, func, PCI_COMMAND_STATUS);
                    let ncmd = cmd | PCI_CMD_IO_SPACE | PCI_CMD_MEM_SPACE | PCI_CMD_BUS_MSTR;
                    pci_write_config(bus, slot, func, PCI_COMMAND_STATUS, ncmd);

                    let bar4_raw = pci_read_config(bus, slot, func, PCI_BAR4);
                ...
                if func == 0 {
                    let header_misc = pci_read_config(bus, slot, func, PCI_HEADER_MISC);
...
{
    let ide = pci_find_ide().ok_or(AtaError("no IDE controller found on PCI bus"))?;
