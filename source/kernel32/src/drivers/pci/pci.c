...
//static struct kernel_io_interfaces kio;
...
void pci_check_device(uint8_t bus, uint8_t device)
{
    ...
    for (uint8_t function = 0; function < total_functions; function++)
    {
        ...
        //kio.serial_print("FOUND DEVICE\n");

        if (base_class == 0x01 && sub_class == 0x01) {
          //  kio.serial_print("FOUND ATA DISK\n");
        }

        char buf[128];
       // kio.snprintf(buf, sizeof(buf), "PCI: Bus %d, Dev %d, Func %d -> Vendor: 0x%04X, Device: 0x%04X, Class: 0x%02X, Sub: 0x%02X\n", 
        ...
        //kio.serial_print(buf);

        if (base_class == 0x03) {
          //  kio.serial_print("	[^] Found Videocard.\n");
        }
    }
}
...
int pci_init(/*struct kernel_io_interfaces *io*/)
{
	//if (!io) return -1;

	//io->register_action_symbol("pci_read_config", (void*)pci_read_config);
	//io->register_action_symbol("pci_write_config", (void*)pci_write_config);

    //io->serial_print("PCI: Initializing and scanning bus...\n");
