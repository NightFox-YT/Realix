...
// Скан-коды клавиш Shift (Left/Right), без бита отпускания
...
// Состояние клавиши Shift (Обновляется в `read_key`, т.к. только там видны release-события)
...
/// Перевод символа в его "сдвинутый" (Shift) вариант
...
pub fn scancode_to_ascii(scancode: u8) -> Option<u8> {
    let base: Option<u8> = scancode_to_base_ascii(scancode);
...
/// Перевод scancode в базовый (не сдвинутый) ASCII-символ
fn scancode_to_base_ascii(scancode: u8) -> Option<u8> {
    match scancode {
        // Буквы
...
pub fn read_key() -> Key {
    ...
    loop {
        if let Some(scancode) = queue_pop() {
            ...
            // Отслеживаем состояние Shift (Нужны и press, и release события)
            let base_code: u8 = scancode & !SCANCODE_RELEASE;
            if base_code == SCANCODE_LSHIFT || base_code == SCANCODE_RSHIFT {
