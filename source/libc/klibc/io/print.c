...
int snprintf (char *str, size_t size, const char *format, ...)
{
	...
	va_start(ap, format);
	rc = kvsnprintf(str, size, format, ap);
	va_end(ap);
...
int kvprintf(const char *format, va_list ap)
{
    ...
    int written = kvsnprintf(buf, PRINTF_BUFFER_SIZE, format, ap);
    
    if (written > 0) sys_puts_impl(buf);
...
int ksprintf(char *str, const char *format, ...)
{
	...
	va_start(ap, format);
	rc = kvsnprintf(str, (size_t)-1, format, ap);
	va_end(ap);
